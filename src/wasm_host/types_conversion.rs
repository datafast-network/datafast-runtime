...
mod tests {
    ...
    host_fn_test!("test", test_bytes_to_hex, host, ptr {
    ...
    host_fn_test!("test", test_bytes_to_string, host, ptr {
        let asc_ptr = AscPtr::<AscString>::new(ptr);
        let string_result: String = asc_get(&host, asc_ptr, 0).unwrap();
        assert_eq!(string_result, "abc");
    });

    host_fn_test!("test", test_hex_to_bytes, host, ptr {
        let asc_ptr = AscPtr::<AscString>::new(ptr);
        let string_result: String = asc_get(&host, asc_ptr, 0).unwrap();
        assert_eq!(string_result, "abc");
    });

    host_fn_test!("test", test_big_int_to_string, host, ptr {
        let asc_ptr = AscPtr::<AscString>::new(ptr);
        let string_result: String = asc_get(&host, asc_ptr, 0).unwrap();
        assert_eq!(string_result, "1000");
    });

    host_fn_test!("test", test_big_int_to_hex, host, ptr {
        let asc_ptr = AscPtr::<AscString>::new(ptr);
        let string_result: String = asc_get(&host, asc_ptr, 0).unwrap();
        assert_eq!(string_result, "0x3e8"); //0x3E8
    });

    host_fn_test!("test", test_bytes_to_base58, host, ptr {
        let asc_ptr = AscPtr::<AscString>::new(ptr);
        let string_result: String = asc_get(&host, asc_ptr, 0).unwrap();
